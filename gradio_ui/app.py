...
def _format_tx_type(tx_type: Any) -> str:
    # tx_type может прийти строкой или enum
    return str(tx_type or "")

def _format_tx_amount(amount: Any, tx_type: str) -> str:
    # Для читаемости: списания показываем со знаком "-"
    try:
        a = int(amount)
    except Exception:
        return str(amount)
    t = (tx_type or "").upper()
    if "CHARGE" in t or "DEBIT" in t or "SPEND" in t or "WITHDRAW" in t:
        return f"-{abs(a)}"
    return f"+{abs(a)}"
...
def ui_transactions(token: str):
    if not token:
        return "❌ Please log in first.", []

    try:
        items = client.get_transactions(token) or []
        if not items:
            return "ℹ️ No transactions yet.", []

        rows = []
        for tx in items:
            tx_type = _format_tx_type(tx.get("tx_type"))
            rows.append(
                [
                    tx.get("id", ""),
                    _format_dt(tx.get("created_at")),
                    tx_type,
                    _format_tx_amount(tx.get("amount"), tx_type)
                ]
            )

        return f"✅ Loaded {len(rows)} transactions.", rows
...
/* wrap description column in tx table */
#tx_df table td:nth-child(5),
#tx_df table th:nth-child(5) {
  white-space: pre-wrap !important;
  word-break: break-word;
}
"""
...
"""
    ...
    ### Transactions
    Click **Load transactions** to view your top-ups and charges history.
    """
        )
            ...
            btn_tx = gr.Button("Load transactions", variant="secondary")
            tx_status = gr.Markdown("", elem_classes=["msgbox"])

            tx_table = gr.Dataframe(
                label="Transactions history",
                headers=["id", "created_at", "type", "amount"],
            ...
            btn_tx.click(ui_transactions, [token_state], [tx_status, tx_table])
